...
namespace XLua
{
    public partial class DelegateBridge : DelegateBridgeBase
    {
		...
		public void __Gen_Delegate_Imp0(string p0, uint p1)
		{
		...
		public bool __Gen_Delegate_Imp10(int p0)
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public void __Gen_Delegate_Imp12(Response p0)
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public override Delegate GetDelegateByType(Type type)
		{
		    ...
		    if (type == typeof(BuffDelete))
			{
			    return new BuffDelete(__Gen_Delegate_Imp0);
			}
			...
			{
			    return new EventCallBack(__Gen_Delegate_Imp8);
			}
		
		    if (type == typeof(RefreshItem))
			{
			    return new RefreshItem(__Gen_Delegate_Imp9);
			}
		
		    if (type == typeof(RequestItemData))
			{
			    return new RequestItemData(__Gen_Delegate_Imp10);
			}
		
		    if (type == typeof(NetMgr.ConnectCallback))
			{
			    return new NetMgr.ConnectCallback(__Gen_Delegate_Imp11);
			}
		
		    if (type == typeof(NetMgr.OnResponse))
			{
			    return new NetMgr.OnResponse(__Gen_Delegate_Imp12);
			}
