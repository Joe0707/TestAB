...
namespace HotfixFrame.Editor.Asset
{
    public class ShaderCollection : EditorWindow
    {
        ...
        readonly public static string ALL_SHADER_VARAINT_PATH = "Assets/ABPackage/ABTest/Shaders/AllShaders.shadervariants";
        readonly public static string ALL_SHADER_AB_PATH = "Shaders/AllShadersInfo";
        #region FindMaterial
        ...
        public static void GenShaderVariant()
        {
            ...
            var paths = HFApplication.GetAllABRootDirects().ToArray();
        ...
        static void AddToDict(Material curMat)
        {
            ...
            for (int i = 0; i < sd.PassTypes.Length; i++)
            {
                ...
                {
                    Debug.LogErrorFormat("{0}-当前shader不存在变体（可以无视）:{1}-{2}", curMat.name, pt, JsonConvert.SerializeObject(curMat.shaderKeywords));
                    continue;
                }
