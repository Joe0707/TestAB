...
{
    private static string sKEY = "ZTdkNTNmNDE2NTM6MWM0uDFhNTEzNzU1";
    private static string sIV = "4rZymeMfa/Ppeq89qY4gyA==";
    ...
    public static void SetBool(string key, bool val)
    {
        ...
        if(val == true)
            tmpVal = 1;
        CryptoPrefs.SetInt(key, tmpVal);
    }
    public static bool GetBool(string key, bool defaultValue = false)
    {
        int readStr = CryptoPrefs.GetInt(key, defaultValue?1:0);
        if(readStr == 1)
            return true;
        else
            return false;
