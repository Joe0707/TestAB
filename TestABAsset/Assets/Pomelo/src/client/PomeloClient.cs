...
namespace Pomelo.DotNetClient
{
    ...
    public class PomeloClient : IDisposable
    {
        ...
        public void initClient(string host, int port, Action callback = null)
        {
            ...
            socket.BeginConnect(host, port, new AsyncCallback((result) =>
            {
                try
                {
                    ...
                    {// ipv4 failed
                        UnityEngine.Debug.LogWarning("IPv4 connection failed. Try use IPv6 connection");
                        this.socket = null;
                        //try IPV6
                        this.socket = new Socket(AddressFamily.InterNetworkV6, SocketType.Stream, ProtocolType.Tcp);
                        this.socket.BeginConnect(host, port, (ar1) =>
                        {
                            if (socket.Connected == false)
                            {//ipv6 failed
                                UnityEngine.Debug.LogError("IPv6 connection failed");
                                NetWorkChanged(NetWorkState.ERROR);
                                Dispose();
                            }
                            else
                            {//ipv6 success 
                                this.socket.EndConnect(ar1);
        ...
        internal void Kick()
        {
            eventManager.InvokeOnEvent(SpecialEventType.Kick, new JsonObject());
        }
